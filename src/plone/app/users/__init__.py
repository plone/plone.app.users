jbot_deprecations = {
    "plone.app.users.browser.account-panel.pt": "plone.app.layout.users.account-panel.pt",
    "plone.app.users.browser.memberregistration.pt": "plone.app.layout.users.memberregistration.pt",
    "plone.app.users.browser.newuser_form.pt": "plone.app.layout.users.newuser_form.pt",
    "plone.app.users.browser.registered.pt": "plone.app.layout.users.registered.pt",
    "plone.app.users.browser.register_form.pt": "plone.app.layout.users.register_form.pt",
    "plone.app.users.browser.schema_layout.pt": "plone.app.layout.users.schema_layout.pt",
}
